...
func (p *MacOSExecProvider) ConfigureNode(ctx context.Context, n *corev1.Node) error {
	...
	n.Status.NodeInfo.ContainerRuntimeVersion = "exec://" + hostInfo.PlatformVersion
...
func (p *MacOSExecProvider) nodeAddresses(ctx context.Context) (addr []corev1.NodeAddress, err error) {
	// If provider doesn't have nodeIPAddress field, we might need a way to store it or recalculate.
	// The previous implementation used p.nodeIPAddress.
	// My MacOSExecProvider struct doesn't have it yet.
	// Let's add it or just calculate it every time (caching is better).
	// For now, let's calculate it.
	ip, err := retrieveNodeIPAddress(ctx)
	if err != nil {
		return nil, err
	}
...
func (p *MacOSExecProvider) nodeDaemonEndpoints() corev1.NodeDaemonEndpoints {
...
// getNodeCapacity returns a resource list containing the capacity limits.
