...
var (
	...
	taintValue  = envOrDefault("VKUBELET_TAINT_VALUE", "macos-exec")
...
func main() {
	...
	desc := binaryName + " implements a node on a Kubernetes cluster using local process execution."
...
func withVersion(cfg *nodeutil.NodeConfig) error {
	cfg.NodeSpec.Status.NodeInfo.KubeletVersion = strings.Join([]string{k8sVersion, "vk-macos-exec", buildVersion}, "-")
	return nil
}
...
func run(ctx context.Context, c kubernetes.Interface) error {
	...
	node, err := nodeutil.NewNode(nodeName,
		func(cfg nodeutil.ProviderConfig) (nodeutil.Provider, node.NodeProvider, error) {
			...
			// Removed docker client creation
			...
			p, err := provider.NewMacOSExecProvider(ctx, processClient, providerConfig)
			if err != nil {
				return nil, nil, err
			}
